"""
This module contains functions for calculating metrics on generated images.
"""
import os
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        with torch.amp.autocast("cuda"):
            ...
            processed_images = torch.stack(
                [model.preprocess(image).to(device) for image in images[i : i + batch_size]]
            )
...
def calculate_scores(
    ...
    pickscore_model = AutoModel.from_pretrained(pickscore_model_name_or_path).eval().to(device)
    ...
    pick_score = calc_pick_or_clip_scores(pickscore_model, image_inputs, text_inputs).mean()

    print("Evaluating CLIP ViT-H-14 score...")
    clip_score = calc_pick_or_clip_scores(clip_model, image_inputs, text_inputs).mean()
...
def distributed_metrics_with_csv(
    ...
    for batch in tqdm(rank_batches, unit="batch", disable=(dist.get_rank() != 0)):
        texts = [str(prompt) for prompt in batch for _ in range(args.num_images_for_metrics)]
        image_tensors = pipe(
            prompt=texts,
            seed=args.seed,
            cfg=args.guidance,
