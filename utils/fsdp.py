...
# Code adapted from:
# https://github.com/yandex-research/switti/blob/master/utils/fsdp.py
...
tdist.new_subgroups = lambda *args, **kwargs: ([], [])
...
def save_model_state(cur_iter: int, args, model: torch.nn.Module):
    ...
    with FSDP.state_dict_type(
        ...
        if dist.is_master():
            ...
            model_save_path = os.path.join(args.local_out_dir_path, "model_state_dict.pt")
...
def save_training_config(args):
    # save as yaml
    with open(os.path.join(args.local_out_dir_path, "training_config.yaml"), "w") as f:
        yaml.dump(args.state_dict(), f)


def save_progress(text_encoder, placeholder_token_ids, args, weight_name, safe_serialization=True):

    print(f"Saving embeddings for {args.placeholder_token}")
    os.makedirs(args.local_out_dir_path, exist_ok=True)
...
def load_model_state(args, model: torch.nn.Module) -> int:
    ...
    if not os.path.exists(model_path):
        ...
        if args.resos[-1] != metadata["args"]["resos"][-1]:
            ...
            C = args.depth * 64
            d = torch.cat([torch.full((pn * pn,), i) for i, pn in enumerate(args.patch_nums)]).view(
                1, L, 1
            )
            dT = d.transpose(1, 2)  # dT: 11L
            model_state_dict["lvl_1L"] = dT[:, 0].contiguous()
            attn_bias_for_masking = torch.where(d >= dT, 0.0, -torch.inf).reshape(1, 1, L, L)
    ...
    tdist.broadcast(start_iter_t, 0)
