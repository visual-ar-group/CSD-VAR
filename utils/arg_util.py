...
# Code adapted from:
# https://github.com/yandex-research/switti/blob/master/utils/arg_util.py
...
class Args(Tap):
    ...
    # pretrained path
    pretrained_path: str = "yresearch/Switti-AR"
    ...
    # num_vectors: int = 1
    num_vectors_content: int = 1
    num_vectors_style: int = 1
    dataset_repeats: int = 10
    num_virtual_tokens_per_scale: int = 20

    # content-style desentanglement
    placeholder_token_content: str = None
    initializer_token_content: str = None
    placeholder_token_style: str = None
    initializer_token_style: str = None
    enable_orthogonal_loss: bool = False
    orthogonal_loss_weight: float = 0.4

    # prompt tuning
    prompt_config_path: str = "config_prompt/config.yaml"

    # major-minor
    major_scale_end: int = 3  # equal attends if major_scale_end = 10
    major_type_scale: str = "style"
    minor_attend_weight: float = 0.0

    proj_path: str = None
    proj_2_path: str = None

    # khoi_request
    use_text_encoder: int = 2
    train_specific_scale: str = "0_1_2_3_4_5_6_7_8_9"
    ...
    commit_id: str = (
        subprocess.check_output(f"git rev-parse HEAD", shell=True).decode("utf-8").strip()
        or "[unknown]"
    )  # [automatically set; don't specify this]
    commit_msg: str = (
        subprocess.check_output(f"git log -1", shell=True).decode("utf-8").strip().splitlines()
        or ["[unknown]"]
    )[
        -1
