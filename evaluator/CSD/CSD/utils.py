...
# Code adapted from:
# https://github.com/learn2phoenix/CSD/blob/main/CSD/utils.py
"""
Misc functions.
...
def convert_state_dict(state_dict):
    """
    Convert state dict keys to match the expected format.
    This function handles common state dict key variations.
    """
    new_state_dict = {}
...
class Metrics(object):
    ...
    def get_map(preds, gts, topk=5):
        ...
        row_cols = [
            np.pad(x, (0, topk + 1 - x.shape[0]), "constant", constant_values=(0, 0))
            for x in row_cols
        ]
        precision = np.asarray(
            [np.repeat(np.arange(topk + 1), x) / np.arange(1, topk + 1) for x in row_cols]
        )
