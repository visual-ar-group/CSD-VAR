...
# Code adapted from:
# https://github.com/yandex-research/switti/blob/master/models/vqvae.py
...
class VQVAE(nn.Module):
    ...
    def idxBl_to_img(
        ...
        for idx_Bl in ms_idx_Bl:
            ...
            pn = round(l**0.5)
            ms_h_BChw.append(
                self.quantize.embedding(idx_Bl).transpose(1, 2).view(B, self.Cvae, pn, pn)
            )
    ...
    def img_to_reconstructed_img(
        ...
        if last_one:
            ...
            return [
                self.decoder(self.post_quant_conv(f_hat)).clamp_(-1, 1) for f_hat in ls_f_hat_BChw
            ]

    # added by me
    def img_to_reconstructed_img_with_each_scale(
        self,
        x,
        v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None,
        last_one=False,
        ...
        (
            ls_f_hat_BChw,
            ls_f_hat_BChw_each_scale,
            ls_reversed_f_hat_BChw,
            ablated_f_hat_BChw_each_scale,
        ) = self.quantize.f_to_idxBl_or_fhat_return_each_scale(
            f, to_fhat=True, v_patch_nums=v_patch_nums
        )

        if last_one:
            if not throuh_decoder:
                return (
                    ls_f_hat_BChw[-1],
                    ls_f_hat_BChw_each_scale[-1],
                    ls_reversed_f_hat_BChw[-1],
                    ablated_f_hat_BChw_each_scale[-1],
                )
            else:
                return (
                    self.decoder(self.post_quant_conv(ls_f_hat_BChw[-1])).clamp_(-1, 1),
                    self.decoder(self.post_quant_conv(ls_f_hat_BChw_each_scale[-1])).clamp_(-1, 1),
                    self.decoder(self.post_quant_conv(ls_reversed_f_hat_BChw[-1])).clamp_(-1, 1),
                    self.decoder(self.post_quant_conv(ablated_f_hat_BChw_each_scale[-1])).clamp_(
                        -1, 1
                    ),
                )
        else:
            if not throuh_decoder:
                ...
                return (
                    ...
                    [
                        self.decoder(self.post_quant_conv(f_hat)).clamp_(-1, 1)
                        for f_hat in ls_f_hat_BChw_each_scale
                    ],
                    [
                        self.decoder(self.post_quant_conv(f_hat)).clamp_(-1, 1)
                        for f_hat in ls_reversed_f_hat_BChw
                    ],
                    [
                        self.decoder(self.post_quant_conv(f_hat)).clamp_(-1, 1)
                        for f_hat in ablated_f_hat_BChw_each_scale
                    ],
                )

    # added by me
    def decode_quantized_feature_map(self, f_hat: torch.Tensor) -> torch.Tensor:
        return self.decoder(self.post_quant_conv(f_hat)).clamp_(-1, 1)
