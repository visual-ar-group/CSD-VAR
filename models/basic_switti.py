...
# Code adapted from:
# https://github.com/yandex-research/switti/blob/master/models/basic_switti.py
...
class SwiGLUFFN(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        return self.down_proj(self._forward_silu_gating(self.gate_proj(x), self.up_proj(x)))
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        q = q.view(B, L, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # BHLc
        k = k.view(B, L, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # BHLc
        v = v.view(B, L, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # BHLc

        if freqs_cis is not None:
            q = apply_rotary_emb(q, freqs_cis=freqs_cis)
            # q[:, :, non_virtual_indices] = apply_rotary_emb(q[:, :, non_virtual_indices], freqs_cis=freqs_cis)

            # k = apply_rotary_emb(k, freqs_cis=freqs_cis)
            # k[:, :, non_virtual_indices] = apply_rotary_emb(k[:, :, non_virtual_indices], freqs_cis=freqs_cis)
            k = apply_rotary_emb(k, freqs_cis=freqs_cis)

        if virtual_kv is not None:

            list_k = []
            list_v = []

            for i in range(len(virtual_kv)):
                # Expand virtual tokens to match the batch size
                virtual_k, virtual_v = virtual_kv[i].view(B, -1, 2, C).unbind(dim=2)

                virtual_k = virtual_k.view(B, -1, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
                virtual_v = virtual_v.view(B, -1, self.num_heads, self.head_dim).permute(0, 2, 1, 3)

                if i in selected_scales:
                    list_k.append(
                        torch.cat(
                            [
                                virtual_k,
                                k[:, :, non_virtual_indices[i][0] : non_virtual_indices[i][1]],
                            ],
                            dim=2,
                        )
                    )
                    list_v.append(
                        torch.cat(
                            [
                                virtual_v,
                                v[:, :, non_virtual_indices[i][0] : non_virtual_indices[i][1]],
                            ],
                            dim=2,
                        )
                    )
                else:
                    list_k.append(k[:, :, non_virtual_indices[i][0] : non_virtual_indices[i][1]])
                    list_v.append(v[:, :, non_virtual_indices[i][0] : non_virtual_indices[i][1]])

            k = torch.cat(list_k, dim=2)
            v = torch.cat(list_v, dim=2)
    ...
    def forward(
        ...
        x = x + self.self_attention_norm2(
            self.attn(
                ...
                non_virtual_indices=non_virtual_indices,
        ...
        x = x + self.ffn_norm2(self.ffn(self.ffn_norm1(x).mul(scale2.add(1)).add(shift2))).mul(
            gamma2
        )
        return x
