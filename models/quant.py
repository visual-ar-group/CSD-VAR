...
# Code adapted from:
# https://github.com/yandex-research/switti/blob/master/models/quant.py
...
class VectorQuantizer2(nn.Module):
    ...
    def forward(
        ...
        margin = (
            tdist.get_world_size() * (f_BChw.numel() / f_BChw.shape[1]) / self.vocab_size * 0.08
        )
        # margin = pn*pn / 100
        if ret_usages:
            usages = [
    ...
    # added by me
    def f_to_idxBl_or_fhat_return_each_scale(
        self,
        f_BChw: torch.Tensor,
        to_fhat: bool,
        v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None,
        ...
        f_hat_or_idx_Bl: List[torch.Tensor] = []
        f_hat_or_idx_Bl_each_scale: List[torch.Tensor] = []
        patch_hws = [
            (pn, pn) if isinstance(pn, int) else (pn[0], pn[1])
            for pn in (v_patch_nums or self.v_patch_nums)
        ]  # from small to large
        ...
        for si, (ph, pw) in enumerate(patch_hws):  # from small to large
            ...
            f_rest.sub_(h_BChw)
            f_hat_or_idx_Bl.append(f_hat.clone() if to_fhat else idx_N.reshape(B, ph * pw))
            f_hat_or_idx_Bl_each_scale.append(h_BChw.clone())

        # reversed_f_hat
        reversed_f_hat = torch.zeros_like(f_hat)
        reversed_f_hat_or_idx_Bl = []
        for i in range(len(f_hat_or_idx_Bl)):
            reversed_f_hat.add_(f_hat_or_idx_Bl_each_scale[-(i + 1)])
            reversed_f_hat_or_idx_Bl.append(reversed_f_hat.clone())

        # ablate each scale
        ablated_f_hat_or_idx_Bl = []
        for i in range(len(f_hat_or_idx_Bl_each_scale)):
            ablated_f_hat_or_idx_Bl.append(f_hat - f_hat_or_idx_Bl_each_scale[i])

        return (
            f_hat_or_idx_Bl,
            f_hat_or_idx_Bl_each_scale,
            reversed_f_hat_or_idx_Bl,
            ablated_f_hat_or_idx_Bl,
        )
